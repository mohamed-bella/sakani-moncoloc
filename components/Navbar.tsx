        ...
        <div className="max-w-[1280px] mx-auto px-4 w-full h-[48px] flex items-center justify-between">
          ...
          <div className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-[#edeff1] h-[64px] z-[100] flex items-center justify-between px-2 pb-safe shadow-[0_-8px_30px_rgba(0,0,0,0.04)]">
            ...
            <Link 
              ...
              className={`flex flex-1 flex-col items-center justify-center h-full transition-all active:scale-90 ${isActive('/profile') || isActive('/auth/register') ? 'text-[#1c1c1c]' : 'text-[#878A8C]'}`}
            >
               <svg className="w-6 h-6 mb-1" fill={isActive('/profile') || isActive('/auth/register') ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={isActive('/profile') || isActive('/auth/register') ? "0" : "2.5"} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" /></svg>
               <span className="text-[9px] font-black uppercase tracking-tighter">{user ? 'حسابي' : 'سجل الآن'}</span>
            </Link>
          </div>
