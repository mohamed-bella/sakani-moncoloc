...
export default function WhatsAppButton({
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      await fetch(`/api/listings/${listingId}/whatsapp`, { method: 'POST' })
    } catch {
    ...
    // Open WhatsApp link
    const url = buildWhatsAppUrl(whatsappNumber, listingTitle)
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className={`btn-primary flex items-center justify-center gap-2 px-4 py-2 text-sm disabled:opacity-70 cursor-pointer ${
        fullWidth ? 'w-full' : ''
      }`}
    >
      {/* WhatsApp Icon */}
      ...
      <span>{loading ? 'جاري الفتح...' : 'تواصل عبر واتساب'}</span>
    </button>
  )
}
