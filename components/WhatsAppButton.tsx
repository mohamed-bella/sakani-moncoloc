...
export default function WhatsAppButton({
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
           // Redirect guest to signup
           window.location.href = `/auth/register?redirectTo=/listing/${listingId}`
           return
        }
  ...
  return (
    <button
      onClick={handleClick}
      disabled={loading}
      className={`bg-[#0079D3] hover:bg-[#0062ab] text-white flex items-center justify-center gap-2 px-4 py-4 rounded-2xl text-[0.95rem] font-black disabled:opacity-70 cursor-pointer shadow-[0_8px_30px_rgb(0,121,211,0.2)] active:scale-95 transition-all outline-none border-none ${
        fullWidth ? 'w-full' : ''
      }`}
    >
      {loading ? (
